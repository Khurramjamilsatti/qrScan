<template>
  <div>
    ...
    <div v-else-if="!menus.length && !editing" class="empty-state">
...
.item-block {
  padding: 0.75rem; background: var(--bg-subtle); border-radius: 0.5rem;
...
.menus-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 1rem; }
.menu-item { position: relative; display: flex; flex-direction: column; gap: 0.75rem; }
...
.menu-item__footer { display: flex; flex-wrap: wrap; align-items: center; gap: 0.5rem; }
.view-stat {
