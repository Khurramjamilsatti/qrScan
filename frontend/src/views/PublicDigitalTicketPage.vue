  ...
  <div class="public-page public-page-light" :style="pageStyle">
    ...
    <div v-else-if="errorType === 'unpublished'" class="page-center">
      <div class="state-card">
        ...
        <h2>{{ t('digitalTickets.notPublishedTitle') }}</h2>
        <p>{{ t('digitalTickets.notPublishedDesc') }}</p>
    ...
    <div v-else-if="errorType === 'notfound'" class="page-center">
      <div class="state-card">
        <div class="state-icon">🔍</div>
        <h2>{{ t('digitalTickets.notFoundTitle') }}</h2>
        <p>{{ t('digitalTickets.notFoundDesc') }}</p>
        <router-link to="/" class="btn-primary">{{ t('common.goToQrScan') }}</router-link>
      </div>
    </div>

    <main v-else class="page-center wide">
      <div class="published-badge"><span class="badge-dot"></span>{{ t('public.digitalTicket') }}</div>
      <article class="public-ticket">
        <TicketRenderer
        ...
        />
        <div class="share-qr">
          <p class="share-label">{{ t('digitalTickets.scanToViewTicket') }}</p>
          <QrPreview
            :content="ticketUrl"
            :name="ticket.event_name"
            :foreground="themeColor"
            :logo-url="ticket.logo_path"
            :background-image="ticket.background_image_path"
            :size="160"
            :qr-shape="ticket.qr_shape || 'square'"
            :dot-style="ticket.dot_style || 'square'"
            :corner-style="ticket.corner_style || 'sharp'"
            :frame-style="ticket.frame_style || 'none'"
          />
        </div>
      </article>
    </main>
...
const themeColor = computed(() => ticket.value?.theme_color || '#e8655a')
const pageStyle = computed(() => ({ '--page-theme': themeColor.value }))
const ticketUrl = computed(() => `${window.location.origin}/ticket/${route.params.slug}`)
...
async function load() {
  ...
  ticket.value = null
  try {
    const { data } = await axios.get(`/api/ticket/${route.params.slug}`)
    ticket.value = data
  } catch (e) {
